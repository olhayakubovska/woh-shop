...
export function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden bg-background">
      <div className="container relative grid gap-6 pt-14 pb-5 md:gap-8 3xl:grid-cols-[395px_200_200_200] 3xl:gap-25 3xl:pt-20 3xl:pb-11.25">
          ...
          <div className="flex gap-4 3xl:mt-8">
            {SOCIAL_LINKS.map(({ label, Icon }) => (
              <Link
                key={label}
                href="#"
                className="flex h-12.5 w-12.5 items-center justify-center rounded-full border border-dark-main/8 bg-white-main text-dark-main transition-all duration-300 hover:-translate-y-2 hover:-rotate-12 hover:border-pink-main hover:bg-pink-main hover:text-white"
                aria-label={label}
              >
                <Icon />
              </Link>
        ...
        <div className="pointer-events-none absolute bottom-0 left-30 z-0 -translate-x-108.75 font-(family-name:--font-urbanist) text-[150px] leading-none font-bold tracking-[-10px] whitespace-nowrap text-watermark select-none md:text-[315px] 3xl:left-90">
          HIGH HEELS
        </div>
      </div>

      <div className="relative left-1/2 hidden w-screen -translate-x-1/2 border-t border-border 3xl:block"></div>
      <div className="container relative flex flex-col items-center gap-6 border-t border-border py-6 text-center text-xs text-text-muted 3xl:flex-row 3xl:items-center 3xl:justify-between 3xl:border-t-0 3xl:text-left">
        <span className="font-golos text-[12px] leading-[100%] font-semibold tracking-[1px] text-text-secondary uppercase">
          © 2026 Structure Lab. Всі права захищені
        </span>
        <div className="flex flex-col items-center gap-6 text-dark-main 3xl:flex-row 3xl:gap-4">
          <Link
            href="#"
